...
function useInfoViewComponents(props: InfoViewProps) {
  return {
    ...
    textColor: props.textColor ?? "#e8edf5",
  };
}

function Question(props: { question: string; textColor: string; children: React.ReactNode }) {
  return (
    <div style={{ marginBottom: 14 }}>
      <div style={{ fontWeight: 900, marginBottom: 4 }}>{props.question}</div>
      <div style={{ color: props.textColor, lineHeight: 1.55 }}>{props.children}</div>
    </div>
  );
}

export function FaqView(props: InfoViewProps) {
  const { PanelComponent, textColor } = useInfoViewComponents(props);
  return (
    <InfoPageLayout title="FAQ" color={textColor}>
      <PanelComponent title="Common questions">
        <Question question="Where is my data stored?" textColor={textColor}>
        ...
        <Question question="Is this a VTT or a rules engine?" textColor={textColor}>
          Neither... Beholden is a campaign tracker + combat helper. It does not try to enforce rules or replace your table.
        </Question>

        <Question question="Can I import monsters?" textColor={textColor}>
        ...
        <Question question="Does Beholden sync to the cloud?" textColor={textColor}>
        ...
        <Question question="Why make this?" textColor={textColor}>
        ...
        <Question question="When do updates come out?" textColor={textColor}>
        ...
        <Question question="Why should I support you?" textColor={textColor}>
        ...
        <Question question="I have a great idea!" textColor={textColor}>
...
export function AboutView(props: InfoViewProps) {
  const { PanelComponent, textColor } = useInfoViewComponents(props);
  return (
    <InfoPageLayout title="About Beholden" color={textColor}>
      <PanelComponent title="What is Beholden?" style={{ marginBottom: 12 }}>
        <div style={{ lineHeight: 1.5, color: textColor }}>
...
export function UpdatesView(props: InfoViewProps) {
  const { PanelComponent, textColor } = useInfoViewComponents(props);
  return (
    <InfoPageLayout title="Future Updates" color={textColor}>
      <PanelComponent title="Roadmap (high level)" style={{ marginBottom: 12 }}>
