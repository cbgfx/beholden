...
function ItemFormModalContent({ item, onClose, onSaved, createOnly }: { item: ItemForEdit | null; onClose: () => void; onSaved: (id?: string) => void; request: ItemEditorRequest; createOnly?: boolean }) {
  ...
  return <div onClick={(event) => { if (event.target === event.currentTarget) onClose(); }} style={{ position: "fixed", inset: 0, zIndex: 1000, background: withAlpha("#000", .72), padding: 12 }}>
    <form onSubmit={save} style={{ width: "min(1240px, 100%)", height: "calc(100vh - 24px)", margin: "auto", background: "rgba(10,18,33,.985)", border: "1px solid rgba(120,150,200,.2)", borderRadius: 14, boxShadow: "0 26px 80px rgba(0,0,0,.58)", display: "flex", flexDirection: "column", overflow: "hidden" }}>
      <header style={{ padding: "14px 20px 0", borderBottom: `1px solid ${theme.colors.panelBorder}` }}><div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", paddingBottom: 12 }}><strong>{create ? translateUi("New Item") : translateUi("Edit: {{value1}}", { value1: item!.name })}</strong><button type="button" aria-label={translateUi("Close")} onClick={onClose} style={{ border: 0, background: "transparent", color: theme.colors.muted, fontSize: 24, cursor: "pointer" }}>×</button></div><nav aria-label={translateUi("Item editor sections")} style={{ display: "flex", gap: 4 }}>{TABS.map((entry) => { const active = tab === entry.id; const count = entry.id === "mechanics" ? mechanicCount : entry.id === "spells" ? form.spells.length : 0; return <button key={entry.id} type="button" onClick={() => setTab(entry.id)} style={{ border: 0, borderBottom: `2px solid ${active ? theme.colors.accentHighlight : "transparent"}`, background: active ? withAlpha(theme.colors.accentHighlight, .08) : "transparent", color: active ? theme.colors.accentHighlight : theme.colors.muted, padding: "9px 14px", cursor: "pointer", fontWeight: 750, borderRadius: "7px 7px 0 0" }}>{translateUi(entry.label)}{count ? ` · ${count}` : ""}</button>; })}</nav></header>
      ...
      <footer style={{ padding: "12px 20px", borderTop: `1px solid ${theme.colors.panelBorder}`, display: "flex", alignItems: "center", justifyContent: "space-between", gap: 8 }}><span style={{ color: theme.colors.muted, fontSize: "var(--fs-small)" }}>{form.name.trim() || "Unnamed item"} · {form.type || "No type"}</span><div style={{ display: "flex", gap: 8 }}><Button type="button" variant="ghost" onClick={onClose} disabled={busy}>{translateUi("Cancel")}</Button><Button type="submit" variant="primary" disabled={busy}>{busy ? translateUi("Saving…") : create ? translateUi("Create Item") : translateUi("Save Changes")}</Button></div></footer>
    </form>
  </div>;
}
