...
type ItemModifierRow = { target: string; amount: string; weaponNames?: string; requiresNoArmor?: boolean; requiresNoShield?: boolean };
...
export type ItemForEdit = {
  ...
  modifiers?: Array<{ target: string; amount: number; weaponNames?: string[]; requiresNoArmor?: true; requiresNoShield?: true }>; rolls?: Array<{ formula: string; description?: string }>;
...
export function buildItemPayload(form: ItemFormData, original: ItemForEdit | null): Record<string, unknown> {
  ...
  const modifiers = form.modifiers.filter((row) => row.target && row.amount).map((row) => { const weaponNames = String(row.weaponNames ?? "").split(",").map((name) => name.trim()).filter(Boolean); return { target: row.target, amount: Number(row.amount), ...(weaponNames.length ? { weaponNames } : {}), ...(row.requiresNoArmor ? { requiresNoArmor: true } : {}), ...(row.requiresNoShield ? { requiresNoShield: true } : {}) }; });
