...
const ClassFeatureSchema = z
  .object({
    ...
    talent: z.object({
      kind: z.enum(["invocation", "maneuver", "metamagic", "infusion"]),
      known: z.record(z.string().regex(/^(?:[1-9]|1[0-9]|20)$/u), z.number().int().positive()),
      replace: z.literal(true).optional(),
    ...
    /** Pure rules-reference/boilerplate card (e.g. "Starting Rogue", "Multiclass Features") with no
     * content specific to this character -- its facts are granted/shown elsewhere. Suppressed from
     * the player-facing Features list, but still a real feature for every other consumer. */
...
const ClassToolProficiencySchema = z
  .object({
