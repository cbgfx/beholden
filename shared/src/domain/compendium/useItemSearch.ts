...
export function useCompendiumItemSearch(
  ...
  const requestPage = React.useCallback(async (offset: number, signal: AbortSignal) => {
    ...
    const hasFacetFilters =
      rarityFilter !== "all" || typeFilter !== "all" || filterAttunement || filterMagic || filterNonMagic;
  ...
  }, [api, filterAttunement, filterMagic, filterNonMagic, includeError, q, rarityFilter, rulesetFilter, typeFilter]);
  ...
  const { rows, totalCount, busy, loadingMore, hasMore, loadMore } = usePaginatedCompendiumSearch({
    enabled,
    refreshKey,
    requestPage,
