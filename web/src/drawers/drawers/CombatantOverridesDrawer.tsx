...
export function CombatantOverridesDrawer(props: {
  ...
  React.useEffect(() => {
    ...
    if (!c) return;
    setInitiative(c.initiative != null ? String(c.initiative) : "");
    setColor(String(c.color ?? ""));
    setFriendly(Boolean(c.friendly));
    const o: any = c.overrides ?? { tempHp: 0, acBonus: 0, hpMaxOverride: null };
  ...
  const showMonsterFields = (combatant as any)?.baseType === "monster";
    ...
    body: (
      <div style={{ display: "grid", gap: 12 }}>
        <div>
          ...
          <Input value={initiative} onChange={(e) => setInitiative(digitsOrEmpty(e.target.value))} placeholder="—" />
        </div>
        ...
        <div>
          <div style={{ color: theme.colors.muted, marginBottom: 6 }}>Max HP override</div>
          <Input value={hpMaxOverride} onChange={(e) => setHpMaxOverride(digitsOrEmpty(e.target.value))} placeholder="—" inputMode="numeric" />
        </div>
      </div>
    ),
    footer: (
      ...
      {options.map((c) => (
        <button
          ...
          style={{
            ...
            background: c,
            boxShadow: props.value === c ? `0 0 0 2px ${theme.colors.accent}` : "none",
            cursor: "pointer"
          }}
        />
      ))}
