  ...
  return (
    <div style={{ display: "grid", gap: 12, minHeight: 0 }}>
      ...
      <Panel title="Import / Export Campaign">
        <div style={{ color: theme.colors.muted, lineHeight: 1.4 }}>
          Campaigns are stored as separate JSON files on disk. Export downloads a single campaign JSON. Import restores (overwrites) a campaign
          with the same <code>campaign.id</code>.
        </div>

        <div style={{ marginTop: 12, display: "flex", gap: 10, alignItems: "center", flexWrap: "wrap" }}>
          <Select value={selectedCampaignId} onChange={(e) => setSelectedCampaignId(e.target.value)} style={{ minWidth: 240 }}>
            {campaigns.map((c) => (
