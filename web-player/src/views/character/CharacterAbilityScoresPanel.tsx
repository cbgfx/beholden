...
export function AbilityScoresPanel({
  ...
  return (
    <CollapsiblePanel
      title={t("abilityScoresPanel.title")}
      color={accentColor}
      storageKey={PANEL_IDS.abilitiesSaves}
      actions={<PanelHeaderAddButton color={accentColor} onClick={onOpenPermanentBuffs} title={t("abilityScoresPanel.permanentBuffs")} />}
    ...
    >
      <AbilityScoresCompact
        ...
        renderScore={({ key }) => {
          ...
          return (
            <Tooltip text={scoreExplanations?.[k] ?? t("abilityScoresPanel.scoreNotSet", { ability: ABILITY_FULL[k], score: score ?? t("abilityScoresPanel.notSet") })} multiline>
          ...
          return (
            <div style={{ fontSize: "var(--fs-subtitle)", fontWeight: 700, textAlign: "center", color: abilityCheckState === "disadvantage" ? C.colorPinkRed : abilityCheckState === "advantage" ? accentColor : C.text, display: "flex", alignItems: "center", justifyContent: "center", gap: 3 }}>
              ...
              <StateBadge
                ...
                accentColor={accentColor}
                title={abilityCheckState === translateUi("advantage")
                  ? t("abilityScoresPanel.advantageOnChecks", { ability: ABILITY_FULL[k] })
                  : t("abilityScoresPanel.disadvantageOnChecks", { ability: ABILITY_FULL[k] })}
              />
            </div>
          );
        }}
        renderSave={({ key }) => {
          ...
          return (
            <div style={{ fontSize: "var(--fs-subtitle)", fontWeight: 700, textAlign: "center", color: saveState === "disadvantage" ? C.colorPinkRed : saveState === "advantage" ? accentColor : isProfSave ? accentColor : C.text, position: "relative", display: "flex", alignItems: "center", justifyContent: "center", gap: 3 }}>
              <Tooltip text={isProfSave
                ? t("abilityScoresPanel.saveBreakdownProficient", { mod: fmtMod(m), pb, total: fmtMod(save) }) + (armorSaveDisadvantage ? t("abilityScoresPanel.disadvantageFromArmorSuffix") : "")
                : t("abilityScoresPanel.saveBreakdownNotProficient", { mod: fmtMod(m) }) + (armorSaveDisadvantage ? t("abilityScoresPanel.disadvantageFromArmorSuffix") : "")}>
                <span style={{ cursor: "help" }}>{fmtMod(save)}</span>
              </Tooltip>
              <StateBadge
                state={saveState}
                accentColor={accentColor}
                title={saveState === translateUi("advantage")
                  ? t("abilityScoresPanel.advantageOnSaves", { ability: ABILITY_FULL[k] })
                  : t("abilityScoresPanel.disadvantageOnSaves", { ability: ABILITY_FULL[k] })}
              />
