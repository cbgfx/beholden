...
import { featureMatchesSubclass } from "@/views/character-creator/utils/CharacterCreatorClassCoreUtils";
...
export function getClassLanguageChoice(
  ...
  for (const autolevel of classDetail.autolevels) {
    ...
    for (const feature of autolevel.features) {
      ...
      if (!featureMatchesSubclass({ name: feature.name, subclass: feature.subclass }, selectedSubclass)) continue;
