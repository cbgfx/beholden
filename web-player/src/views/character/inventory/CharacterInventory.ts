...
import type {
  ...
  ParsedItemSpell,
  ProficiencyMapLike,
} from "@/views/character/inventory/CharacterInventoryTypes";

export * from "@/views/character/inventory/CharacterInventoryTypes";
...
export function mergeCatalogItem(item: InventoryItem, summary: ItemSummaryRow, chargesMax: number | null): InventoryItem {
  const isLinked = item.source !== "custom";
  ...
  const merged: InventoryItem = {
    ...
    source: item.source === "custom" ? item.source : "compendium",
...
function weaponMatchesProficiency(item: InventoryItem, proficiencyName: string): boolean {
  ...
  // Class/feat text grants proficiency with specific weapons in plural form ("Rapiers"), while an
  // item's own base-weapon identity (name and `proficiency` field) is singular ("Rapier") -- compare
  // both forms so a plural grant still matches the singular item.
  const singularNormalized = normalizeWeaponProficiencyName(singularizeInventoryLookupName(proficiencyName)).toLowerCase();
  ...
  if (!normalized) return false;
  if (normalized === itemName || singularNormalized === itemName) return true;
  // `baseWeapon` comes from the item's `proficiency` field, not its display name, so this match
  // survives the player renaming the item (e.g. a Rapier +3 renamed to a custom weapon name).
  if (baseWeapon && (normalized === baseWeapon || singularNormalized === baseWeapon)) return true;
