...
describe("character inventory calculations", () => {
  ...
  it("matches a plural weapon-proficiency grant (e.g. the Rogue's 'Rapiers') against a renamed item's base proficiency", () => {
    const renamedRapier = item({ name: "Tide Turner", dmg1: "1d8", properties: ["M", "F"], proficiency: "martial, rapier" });
    expect(hasWeaponProficiency(renamedRapier, { weapons: [{ name: "Rapiers", source: "Rogue" }], armor: [], weaponMasteries: [] })).toBe(true);
    expect(hasWeaponProficiency(renamedRapier, { weapons: [{ name: "Longswords", source: "Rogue" }], armor: [], weaponMasteries: [] })).toBe(false);
  });
...
describe("mergeCatalogItem", () => {
  ...
  it("preserves edits to a player-owned copy that retains its catalog item id", () => {
    const renamed = item({
      ...
      rarity: "very rare",
      dmg1: "1d10",
      description: "A character-specific version.",
    });
    const catalog = summary({
      id: "i_rapier_plus_3",
      name: "Rapier +3",
    ...
    const merged = mergeCatalogItem(renamed, catalog, null);

    expect(merged.name).toBe("Tide Turner");
    expect(merged.dmg1).toBe("1d10");
    expect(merged.description).toBe("A character-specific version.");
    expect(merged.itemId).toBe("i_rapier_plus_3");
    expect(merged.source).toBe("custom");
  });
});
