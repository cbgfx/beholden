...
import type { ClassRestDetail } from "@/views/character/spells/SpellSlotsPanel";
import { normalizeAbilityKey, normalizeSpellTrackingKey, normalizeSpellTrackingName } from "@/views/character/CharacterSheetUtils";
import { AddSpellDrawer, SpellDrawer } from "@/views/character/spells/CharacterSpellDrawers";
import {
...
} from "@/views/character/spells/CharacterSpellShared";
import { CharacterSpellsGrantedSection } from "@/views/character/spells/CharacterSpellsGrantedSection";
import { CharacterSpellRow } from "@/views/character/spells/CharacterSpellRow";
...
export function RichSpellsPanel({ spells, grantedSpells = [], resources = [], pb, scores, accentColor, classDetail, ruleset, spellSlotState, classSpellcastingStates = [], charLevel, preparedLimit = 0, usesFlexiblePreparedList = false, usedSpellSlots, preparedSpells, onSlotsChange, onPreparedChange, onAddSpell, onRemoveSpell, addSpellSourceLabel, onResourceChange, spellcastingBlocked = false, spellDamageBonuses = {}, spellSaveDcBonus = 0, conditions = [], onToggleCondition }: {
      ...
      if (directIds.length > 0 || unresolvedNames.length > 0) {
        try {
          const payload = await api<{ rows: SpellLookupRow[] }>("/api/spells/lookup", {
            ...
            body: JSON.stringify({ ids: directIds, names: unresolvedNames, includeText: true, ruleset: ruleset ?? undefined }),
          });
  ...
  }, [details, entries, specialGrantedEntries, ruleset]);
  ...
  React.useEffect(() => {
    ...
    api<Array<FetchedSpellDetail & { text?: string | null }>>(`/api/spells/search?q=${encodeURIComponent(query)}&limit=20&lite=1&includeText=1&excludeSpecial=1${rulesetParam}`)
      .then((results) => {
        if (!alive) return;
        setSpellSearchResults(
