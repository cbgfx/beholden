  ...
  return (
    <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 10 }}>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
        ...
        <button
          type="button"
          title={translateUi("Reset to default")}
          aria-label={translateUi("Reset {{value1}} to default", { value1: props.label })}
    ...
    <>
      ...
      >
        <div style={{ padding: "20px 24px", borderBottom: "1px solid rgba(255,255,255,0.08)", display: "flex", alignItems: "center", justifyContent: "space-between", gap: 12 }}>
          <div>
            <div style={{ fontSize: "var(--fs-hero)", fontWeight: 900, color: C.text, marginBottom: 4 }}>{translateUi("Theme")}</div>
            <div style={{ fontSize: "var(--fs-subtitle)", color: C.muted }}>{translateUi("Sheet color, panel colors, and background.")}</div>
          </div>
        ...
        <div style={{ flex: 1, minHeight: 0, overflowY: "auto", WebkitOverflowScrolling: "touch", padding: 24, display: "flex", flexDirection: "column", gap: 24 }}>
          <div>
            <div style={{ padding: "10px 12px", borderRadius: 12, background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.08)" }}>
              <div style={{ fontSize: "var(--fs-tiny)", color: C.muted, marginBottom: 8, textTransform: "uppercase", letterSpacing: "0.08em" }}>{translateUi("Sheet color")}</div>
            ...
            <div style={{ marginTop: 12, display: "grid", gap: 12, padding: "12px", borderRadius: 12, background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.08)" }}>
              <ColorPickerRow
                label={translateUi("Sheet background")}
              ...
              />
              <ColorPickerRow
                label={translateUi("Panel background")}
              ...
              />
              <div>
                <div style={{ fontSize: "var(--fs-tiny)", color: C.muted, marginBottom: 8, textTransform: "uppercase", letterSpacing: "0.08em" }}>{translateUi("Background pattern")}</div>
                ...
                <label style={{ display: "grid", gridTemplateColumns: "1fr auto", gap: 8, alignItems: "center", marginTop: 10, color: C.muted }}>
                  <input aria-label={translateUi("Background intensity")} type="range" min={0} max={100} value={props.appearanceDraft.backgroundIntensity ?? 0} onChange={(event) => props.onAppearanceChange({ ...props.appearanceDraft, backgroundIntensity: Number(event.target.value) })} />
        ...
        <div style={{ padding: "24px 24px calc(24px + env(safe-area-inset-bottom, 0px))", borderTop: "1px solid rgba(255,255,255,0.08)", display: "flex", justifyContent: "flex-end", gap: 12, flexShrink: 0 }}>
          ...
          >
            {props.saving ? translateUi("Saving...") : translateUi("Save")}
          </Button>
        </div>
      </div>
    </>
