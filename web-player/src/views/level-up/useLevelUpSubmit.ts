...
type ManeuverChoiceEntry = {
  definition: { category: "maneuver" | "metamagic" | "infusion" | "plan"; sourceLabel: string; sourceKey?: string | null };
  ...
  const confirm = React.useCallback(async () => {
    ...
    try {
      ...
      const selectedFeatureProficiencyEntries = classFeatureProficiencyChoices.reduce<Partial<Record<"skills" | "tools" | "languages" | "armor" | "weapons" | "saves", Array<{ name: string; source: string }>>>>((acc, choice) => {
        ...
        if (choice.category === "armor") acc.armor = [...(acc.armor ?? []), ...selected];
        if (choice.category === "weapon") acc.weapons = [...(acc.weapons ?? []), ...selected];
        if (choice.category === "saving_throw") acc.saves = [...(acc.saves ?? []), ...selected];
        return acc;
      }, {});
      ...
      const selectedInfusionEntries = maneuverChoiceEntries
        .filter((entry) => entry.definition.category === "infusion")
        .flatMap((entry) =>
          entry.chosenEntries.map((spell) => ({
            id: String(spell.id),
            name: spell.name,
