...
function Tag({ children, color = C.accentHl }: { children: React.ReactNode; color?: string }) {
  return (
    <span style={{
...
export function FeatDetailPanel({ featId }: { featId: string }) {
  ...
  return (
    <Panel
      ...
      actions={<div style={{ color: C.muted, fontSize: "var(--fs-small)" }}>{busy ? "Loading..." : parsed?.source ?? ""}</div>}
      ...
      {!feat ? (
        ...
        <>
          <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
            ...
            {feat.ruleset && <Tag color={C.colorMagic}>{feat.ruleset}</Tag>}
            {parsed?.repeatable && <Tag color={C.colorGold}>Repeatable</Tag>}
            {abilityIncreases.map((value) => <Tag key={value} color={C.green}>{value}</Tag>)}
          </div>

          {feat.prerequisite && (
            <div style={{ padding: "8px 10px", borderRadius: 9, border: `1px solid ${C.panelBorder}`, color: C.muted, fontSize: "var(--fs-small)" }}>
              <b style={{ color: C.text }}>Prerequisite:</b> {feat.prerequisite}
            </div>
          )}
          ...
          <div style={{
            ...
            {feat.text || <span style={{ color: C.muted }}>No description available.</span>}
          </div>
        </>
      )}
    </Panel>
