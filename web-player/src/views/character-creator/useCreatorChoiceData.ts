...
export function useCreatorChoiceData(args: {
  ...
  const { step6ResolvedSpellChoices, growthChoiceDefinitions, ruleset } = args;
  ...
  React.useEffect(() => {
    ...
    let cancelled = false;
    loadSpellChoiceOptions(step6ResolvedSpellChoices, (query) => api<SpellSummary[]>(query), { forceIncludeText: true, ruleset })
      .then((optionsByKey) => {
  ...
  }, [step6ResolvedSpellChoices, ruleset]);
