  ...
  React.useEffect(() => {
    ...
    fetchMyCharacter(editId)
      .then((ch) => {
        ...
        const recordedBgBonuses = cd.bgAbilityBonuses && typeof cd.bgAbilityBonuses === "object" ? cd.bgAbilityBonuses : {};
        ...
        const fallbackBaseScores = {
          str: Math.max(8, (ch.strScore ?? 10) - (Number(recordedBgBonuses.str ?? 0) || 0) - (Number(recordedAsiBonuses.str ?? 0) || 0)),
          dex: Math.max(8, (ch.dexScore ?? 10) - (Number(recordedBgBonuses.dex ?? 0) || 0) - (Number(recordedAsiBonuses.dex ?? 0) || 0)),
          con: Math.max(8, (ch.conScore ?? 10) - (Number(recordedBgBonuses.con ?? 0) || 0) - (Number(recordedAsiBonuses.con ?? 0) || 0)),
          int: Math.max(8, (ch.intScore ?? 10) - (Number(recordedBgBonuses.int ?? 0) || 0) - (Number(recordedAsiBonuses.int ?? 0) || 0)),
          wis: Math.max(8, (ch.wisScore ?? 10) - (Number(recordedBgBonuses.wis ?? 0) || 0) - (Number(recordedAsiBonuses.wis ?? 0) || 0)),
          cha: Math.max(8, (ch.chaScore ?? 10) - (Number(recordedBgBonuses.cha ?? 0) || 0) - (Number(recordedAsiBonuses.cha ?? 0) || 0)),
        };
        const savedAbilityMethod = cd.abilityMethod === "pointbuy" || cd.abilityMethod === "standard"
          ? cd.abilityMethod
        ...
        setForm((f) => ({
          ...
          standardAssign: savedAbilityMethod === "standard"
            ? (cd.standardAssign && typeof cd.standardAssign === "object" ? cd.standardAssign : (inferredStandardAssign ?? f.standardAssign))
            : f.standardAssign,
          pbScores: savedAbilityMethod === "pointbuy"
            ? (cd.pbScores && typeof cd.pbScores === "object" ? cd.pbScores : fallbackBaseScores)
            : f.pbScores,
