...
interface GrowthFeatureLike {
  ...
  talent?: { kind: "invocation" | "maneuver" | "metamagic" | "infusion"; known: Record<string, number>; replace?: true; ability?: string[] } | null;
}
...
export interface GrowthChoiceDefinition {
  key: string;
  sourceKey: string;
  category: "maneuver" | "metamagic" | "infusion" | "plan";
  title: string;
  sourceLabel: string;
  totalCount: number;
...
const TALENT_GROWTH_KIND: Record<"maneuver" | "metamagic" | "infusion", { category: "maneuver" | "metamagic" | "infusion"; title: string; sourceKeySuffix: string }> = {
  ...
  infusion: { category: "infusion", title: "Infusions", sourceKeySuffix: "infusions" },
};
...
export function getGrowthChoiceDefinitions(args: {
  ...
  for (const row of classDetail.autolevels) {
    ...
    for (const feature of row.features ?? []) {
      ...
      if (talent?.kind !== "maneuver" && talent?.kind !== "metamagic" && talent?.kind !== "infusion") continue;
...
export function sanitizeGrowthChoiceSelections(args: {
  ...
  for (const key of Object.keys(next)) if (!validKeys.has(key) && (key.includes(":maneuvers") || key.includes(":metamagic") || key.includes(":infusions") || key.includes(":plans"))) delete next[key];
  return next;
}
