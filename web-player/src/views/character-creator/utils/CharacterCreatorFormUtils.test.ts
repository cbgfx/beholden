...
describe("getPrimaryAbilityKeys", () => {
  it("resolves a single short-key primary ability (compendium storage format for both rulesets)", () => {
    expect(getPrimaryAbilityKeys(classDetail({ primaryAbility: "str" }))).toEqual(["str"]);
  });

  it("resolves an 'any' pair of short-key abilities", () => {
    expect(getPrimaryAbilityKeys(classDetail({ primaryAbility: { any: ["str", "dex"] } }))).toEqual(["str", "dex"]);
  });

  it("resolves an 'all' pair of short-key abilities", () => {
    expect(getPrimaryAbilityKeys(classDetail({ primaryAbility: { all: ["dex", "wis"] } }))).toEqual(["dex", "wis"]);
  });
