...
export interface SharedResolvedSpellChoiceEntry {
  ...
  talentKind?: "invocation" | "maneuver" | "metamagic" | "infusion";
}
...
export async function loadSpellChoiceOptions(
  ...
  options?: { excludeSpecial?: boolean; forceIncludeText?: boolean; ruleset?: "5e" | "5.5e" | null },
): Promise<Record<string, SharedSpellSummary[]>> {
  const excludeSpecial = options?.excludeSpecial ?? true;
  const rulesetParam = options?.ruleset ? `&ruleset=${encodeURIComponent(options.ruleset)}` : "";
  const buildSearchQuery = (args: {
    ...
    return `/api/spells/search?${params.join("&")}${rulesetParam}`;
  };
  ...
  const entries = await Promise.all(
    choices.map(async (choice) => {
      ...
      if (choice.talentKind) {
        const talents = await fetchSpells(`/api/class-talents/search?kind=${choice.talentKind}&limit=150&includeText=1${rulesetParam}`).catch(() => []);
