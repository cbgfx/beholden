...
export function getClassExpertiseChoices(cls: CreatorClassDetailLike | null, level: number, selectedSubclass?: string | null): ClassExpertiseChoice[] {
  if (!cls) return [];
  ...
  for (const al of getMergedAutolevels(cls)) {
    ...
    for (const feature of al.features ?? []) {
      ...
      if (!featureMatchesSubclass(feature, selectedSubclass)) continue;
