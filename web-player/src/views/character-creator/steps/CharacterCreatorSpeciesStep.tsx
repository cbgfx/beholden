...
interface RaceDetailLike {
  ...
  traits: Array<{ name: string; text: string; modifier: string[]; preparedSpellProgression?: PreparedSpellProgressionTable[]; effects?: unknown[]; hidden?: boolean }>;
}
...
function renderSpeciesStep({
  ...
  // Languages the species already grants for free (e.g. 2014 Human's fixed Common) -- excluded
  // from the language choice picker below so they can't be picked again, wasting the real choice.
        ...
        <div style={{ marginTop: 20, display: "flex", flexDirection: "column", gap: 18 }}>
          ...
          {languageChoice && (
            <div>
              ...
              {fixedRaceLanguages.length > 0 && (
                <div style={{ display: "flex", flexWrap: "wrap", gap: 6, marginBottom: 10 }}>
                  {fixedRaceLanguages.map((language) => <span key={language} style={profChipStyle}>{language}</span>)}
                </div>
              )}
              <div style={{ display: "flex", flexWrap: "wrap", gap: 6 }}>
                {(languageChoice.from ?? allLanguages).filter((lang) => !fixedRaceLanguages.some((known) => known.toLowerCase() === lang.toLowerCase())).map((lang) => {
