...
import { normalizeChoiceKey, type StartingEquipmentOption } from "../utils/CharacterCreatorClassCoreUtils";
            ...
            {(languages.fixed.length > 0 || languages.choose > 0) && (
              <div>
                ...
                {languages.fixed.length > 0 && (
                  <div style={{ display: "flex", flexWrap: "wrap", gap: 4, marginBottom: languages.choose > 0 ? 8 : 0 }}>
                    {languages.fixed.map((language) => (
                  ...
                  <>
                    ...
                    <div style={{ display: "flex", flexWrap: "wrap", gap: 6 }}>
                      {(languages.from ?? ALL_LANGUAGES).map((language) => {
                        ...
                        // Already known from species (fixed grant or the species' own language
                        // pick) -- picking it again here would waste this choice on nothing new.
                        const alreadyKnown = !selected && raceLanguageKeys.has(normalizeChoiceKey(language));
                        const locked = alreadyKnown || (!selected && form.chosenBgLanguages.length >= languages.choose);
                        return (
                          <button
                            key={language}
                            type="button"
...
export function renderBackgroundFromContext(ctx: CharacterCreatorStepRenderContext): StepRenderResult {
  ...
  const raceTraitEffects = ctx.raceDetail ? parseAppliedSpeciesTraitEffects(ctx.raceDetail, ctx.form.chosenFeatureChoices) : [];
  const fixedRaceLanguages = collectTaggedGrantsFromEffects(raceTraitEffects).languages.map((entry) => entry.name);
  const raceLanguageKeys = new Set<string>(
    [...fixedRaceLanguages, ...ctx.form.chosenRaceLanguages].map(normalizeChoiceKey)
  );
