...
import { buildAppliedCharacterFeatures, buildDisplayPlayerFeatures } from "./characterFeatures";
...
describe("class-aware feature progression", () => {
  ...
  it("keeps hidden reference-card features out of the player-facing display list but leaves them applied", () => {
    const args = {
      charData: { classes: [] },
      characterLevel: 1,
      classDetail: null,
      classSelections: [
        {
          entry: { id: "rogue-entry", level: 1, subclass: null },
          detail: {
            id: "c_rogue",
            autolevels: [{
              level: 1,
              features: [
                { name: "Starting Rogue", text: "Reference card.", resolution: "manual" as const, hidden: true },
                { name: "Expertise", text: "Choose two skills.", resolution: "automatic" as const, hidden: true },
                { name: "Sneak Attack", text: "Extra damage on hit." },
              ],
            }],
          },
        },
    ...
    expect(applied.map((feature) => feature.name)).toEqual(["Starting Rogue", "Expertise", "Sneak Attack"]);
