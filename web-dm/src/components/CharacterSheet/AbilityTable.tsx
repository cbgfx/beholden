...
function AbilityGroup({
  ...
  keys: AbilityKey[];
  abilities: CharacterSheetStats["abilities"];
  saves?: CharacterSheetStats["saves"];
  compact?: boolean;
}) {
  return (
    <div style={{ minWidth: 0 }}>
      <div style={{ display: "grid", gridTemplateColumns: cols, gap: compact ? 4 : 6, marginBottom: compact ? 4 : 6 }}>
        <div />
      ...
      <div style={{ display: "grid", gap: compact ? 3 : 5 }}>
        {keys.map((k) => {
          ...
          return (
            <div key={k} style={{ display: "grid", gridTemplateColumns: cols, gap: compact ? 4 : 6, alignItems: "center" }}>
              <div style={labelStyle}>{k}</div>
              <div style={{ ...scorePillStyle, padding: compact ? "4px 6px" : scorePillStyle.padding }}>{score}</div>
...
export function AbilityTable({ stats, compact }: { stats: CharacterSheetStats; compact?: boolean }) {
  ...
  return (
    <div style={{ display: "grid", gridTemplateColumns: `repeat(auto-fit, minmax(${compact ? 200 : 220}px, 1fr))`, gap: compact ? 12 : 16, padding: compact ? "4px 0" : "10px 2px" }}>
      <AbilityGroup keys={left} abilities={stats.abilities} saves={stats.saves} compact={compact} />
      <AbilityGroup keys={right} abilities={stats.abilities} saves={stats.saves} compact={compact} />
    </div>
  );
}
