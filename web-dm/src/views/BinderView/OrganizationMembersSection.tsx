...
export function OrganizationMembersSection(props: { binderId: string; organizationId: string; count: number }) {
  ...
  return <section>
    <div style={{ color: theme.colors.muted, fontSize: "var(--fs-small)", fontWeight: 750, textTransform: "uppercase", letterSpacing: "0.06em" }}>{translateUi("Members")}</div>
    {props.count ? <button type="button" onClick={() => void toggle()} aria-expanded={open} style={{ marginTop: 7, padding: 0, border: 0, background: "transparent", color: theme.colors.text, cursor: "pointer", font: "inherit", fontSize: "var(--fs-body)", textDecoration: "underline", textUnderlineOffset: 3 }}>{props.count} {open ? "▴" : "▾"}</button> : <div style={{ fontSize: "var(--fs-body)", marginTop: 7 }}>{translateUi("None")}</div>}
