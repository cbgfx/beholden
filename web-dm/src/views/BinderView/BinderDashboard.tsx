...
function RecordLinks({ rows }: { rows: Array<{ id: string; name: string; type?: string; route: string }> }) {
  ...
  return <div style={{ display: "grid", gap: 7 }}>{rows.map((row) => <Link key={row.id} to={row.route} style={{ display: "flex", justifyContent: "space-between", color: theme.colors.text, textDecoration: "none" }}><span>{row.name}</span>{row.type ? <span style={{ color: theme.colors.muted, textTransform: "capitalize" }}>{row.type}</span> : null}</Link>)}{!rows.length ? <span style={{ color: theme.colors.muted }}>{translateUi("Nothing to review.")}</span> : null}</div>;
}
...
export function BinderDashboardView({ binderId, accent, canEdit }: { binderId: string; accent: string; canEdit: boolean }) {
  ...
  if (!data) return <div style={{ color: theme.colors.muted }}>{translateUi("Loading dashboard…")}</div>;
