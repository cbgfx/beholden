...
export function PlacesWorkspace(props: { binderId: string; accent: string; canEdit: boolean; onRecordsChanged: () => Promise<void> }) {
  ...
  const reload = useCallback(async () => {
    ...
    } catch (cause) { setError(cause instanceof Error ? cause.message : translateMessage("Unable to load Places.")); }
    ...
    <div style={{ display: "grid", gap: 8 }}>
      <div style={{ display: "flex", justifyContent: "space-between", gap: 10, alignItems: "center", flexWrap: "wrap" }}>
        <div style={{ display: "flex", gap: 8, flex: "1 1 520px" }}>
          <Input value={query} onChange={(event) => setQuery(event.target.value)} placeholder={translateUi("Search places…")} style={{ width: "min(360px,100%)" }}/>
          <select value={typeFilter} onChange={(event) => setTypeFilter(event.target.value as typeof typeFilter)} style={{ minWidth: 170, padding: "9px 11px", borderRadius: theme.radius.control, border: `1px solid ${theme.colors.panelBorder}`, background: theme.colors.inputBg, color: theme.colors.text, font: "inherit" }}>
            <option value="all">{translateUi("All place types")}</option>{PLACE_TYPES.map((type) => <option key={type} value={type}>{LABELS[type].plural}</option>)}
          </select>
        </div>
        {props.canEdit ? <div style={{ display: "flex", gap: 8 }}>
          ...
          <Button onClick={() => setCreating(true)}><span style={{ display: "inline-flex", alignItems: "center", gap: 7 }}><IconPlus size={14}/> {translateUi("New Place")}</span></Button>
        </div> : null}
      </div>
      <div style={{ border: `1px solid ${theme.colors.panelBorder}`, borderRadius: theme.radius.panel, overflow: "hidden" }}>
        <div style={{ display: "grid", gridTemplateColumns: "minmax(240px,1.4fr) 170px minmax(220px,1fr) 110px", gap: 10, padding: "8px 12px", background: withAlpha(props.accent,.08), borderBottom: `1px solid ${theme.colors.panelBorder}`, fontSize: "var(--fs-small)", fontWeight: 750 }}><span style={{ color: props.accent }}>{translateUi("Name ▲")}</span><span>{translateUi("Type")}</span><span>{translateUi("Parent")}</span><span>{translateUi("Related")}</span></div>
        {loading ? <div style={{ padding: 42, textAlign: "center", color: theme.colors.muted }}>{translateUi("Loading…")}</div> : error ? <div style={{ padding: 42, textAlign: "center", color: theme.colors.red }}>{error}</div> : visible.length ? visible.map((row) => <button key={row.id} type="button" onClick={() => navigate(`/binder/${props.binderId}/${row.referenceType}/${row.id}`)} style={{ width: "100%", display: "grid", gridTemplateColumns: "minmax(240px,1.4fr) 170px minmax(220px,1fr) 110px", gap: 10, alignItems: "center", padding: "7px 12px", border: 0, borderBottom: `1px solid ${theme.colors.panelBorder}`, background: "transparent", color: theme.colors.text, textAlign: "left", cursor: "pointer", font: "inherit", fontSize: "var(--fs-small)" }}>
        ...
        </button>) : <div style={{ padding: 48, textAlign: "center", color: theme.colors.muted }}>{translateUi("No Places match the current search.")}</div>}
      </div>
    </div>
