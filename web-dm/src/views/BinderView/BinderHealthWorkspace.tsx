...
export function BinderHealthWorkspace({ binderId, accent }: { binderId: string; accent: string }) {
  ...
  if (!health) return <div style={{ color: theme.colors.muted }}>{translateUi("Checking Binder health…")}</div>;
  return <div style={{ display: "grid", gap: 14 }}>
    <div style={{ padding: 18, borderRadius: theme.radius.panel, border: `1px solid ${withAlpha(health.healthy ? theme.colors.green : accent,.35)}`, background: withAlpha(health.healthy ? theme.colors.green : accent,.07) }}>
      <strong style={{ fontSize: "var(--fs-title)" }}>{health.healthy ? translateUi("Binder looks healthy") : translateUi("{{value1}} item{{value2}} to review", { value1: health.issueCount, value2: health.issueCount === 1 ? "" : "s" })}</strong>
      <div style={{ color: theme.colors.muted, marginTop: 5 }}>{translateUi("Diagnostics are non-destructive. Open a record to decide how it should be corrected.")}</div>
    </div>
    {health.groups.map((group) => <section key={group.code} style={{ border: `1px solid ${theme.colors.panelBorder}`, borderRadius: theme.radius.panel, overflow: "hidden", background: theme.colors.panelBg }}>
      ...
      {!group.issues.length ? <div style={{ padding: "12px 14px", color: theme.colors.muted }}>{translateUi("No issues found.")}</div> : null}
    </section>)}
  </div>;
}
