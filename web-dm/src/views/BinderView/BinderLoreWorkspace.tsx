  ...
  return <div style={{ position: "fixed", inset: 0, zIndex: 2000, background: "rgba(0,0,0,.68)" }} onMouseDown={(event) => { if (event.target === event.currentTarget) props.onClose(); }}>
    <aside style={{ position: "absolute", right: 0, top: 0, bottom: 0, width: "min(720px, 95vw)", overflowY: "auto", padding: 22, background: "#0d1729", borderLeft: `1px solid ${theme.colors.panelBorder}`, boxShadow: "-18px 0 50px rgba(0,0,0,.45)" }}>
      <header style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 22 }}>
        <h2 style={{ margin: 0 }}>{props.title}</h2><Button variant="ghost" onClick={props.onClose}>{translateUi("Close")}</Button>
      </header>
      {props.children}
    </aside>
  </div>;
...
function ItemEditor(props: {
  ...
  return <Drawer title={props.item ? translateUi("Edit Item") : translateUi("New Item")} onClose={props.onClose}>
    <div style={{ display: "grid", gap: 14 }}>
      <Field label={translateUi("Name")}><Input autoFocus value={name} onChange={(event) => setName(event.target.value)} /></Field>
      <Field label={translateUi("Compendium item")}><Select value={compendiumItemId} onChange={setCompendiumItemId} options={props.compendium} /></Field>
      <Field label={translateUi("Current holder")}><Select value={holderMortalId} onChange={setHolderMortalId} options={mortals} /></Field>
      <Field label={translateUi("Location")}><Select value={locationRecordId} onChange={setLocationRecordId} options={places} /></Field>
      <div><strong>{translateUi("Description")}</strong><WysiwygNoteEditor value={description} onChange={setDescription} mentions={mentions} minHeight={180} theme={{ radius: theme.radius.control, panelBorder: theme.colors.panelBorder, inputBg: theme.colors.inputBg, text: theme.colors.text }} /></div>
      <div><strong>{translateUi("DM Notes")}</strong><WysiwygNoteEditor value={dmNotes} onChange={setDmNotes} mentions={mentions} minHeight={130} theme={{ radius: theme.radius.control, panelBorder: theme.colors.panelBorder, inputBg: theme.colors.inputBg, text: theme.colors.text }} /></div>
...
function EventEditor(props: {
  ...
  const mentions = props.records.map((row) => ({ id: row.id, label: row.name, href: row.route, type: row.type }));
  return <Drawer title={props.event ? translateUi("Edit Event") : translateUi("New Event")} onClose={props.onClose}>
    <div style={{ display: "grid", gap: 14 }}>
      <Field label={translateUi("Title")}><Input autoFocus value={title} onChange={(event) => setTitle(event.target.value)} /></Field>
      <Field label={translateUi("Date")}><Input value={dateText} onChange={(event) => setDateText(event.target.value)} placeholder={translateUi("None")} /></Field>
      <Field label={translateUi("End date")}><Input value={endDateText} onChange={(event) => setEndDateText(event.target.value)} placeholder={translateUi("None")} /></Field>
      <Field label={translateUi("Tags")}><Input value={tags} onChange={(event) => setTags(event.target.value)} placeholder={translateUi("War, politics, prophecy")} /></Field>
      <Field label={translateUi("Related records")}><MultiRecords options={props.records.filter((row) => row.id !== props.event?.id)} value={records} onChange={setRecords} /></Field>
      <Field label={translateUi("Campaigns")}><MultiRecords options={campaignRecords} value={campaigns} onChange={setCampaigns} /></Field>
      <div><strong>{translateUi("Description")}</strong><WysiwygNoteEditor value={description} onChange={setDescription} mentions={mentions} minHeight={260} theme={{ radius: theme.radius.control, panelBorder: theme.colors.panelBorder, inputBg: theme.colors.inputBg, text: theme.colors.text }} /></div>
      <div style={{ display: "flex", justifyContent: "flex-end", gap: 8 }}><Button variant="ghost" onClick={props.onClose}>{translateUi("Cancel")}</Button>
...
export function BinderLoreWorkspace(props: {
  ...
  if (selected) {
    return <div style={{ display: "grid", gap: 15 }}>
      <button type="button" onClick={() => navigate(`/binder/${props.binderId}/${props.type}`)} style={{ width: "fit-content", border: 0, background: "transparent", color: theme.colors.muted, cursor: "pointer" }}>{translateUi("← All")} {props.type}</button>
      <article style={{ border: `1px solid ${theme.colors.panelBorder}`, borderTop: `4px solid ${props.accent}`, borderRadius: theme.radius.panel, background: theme.colors.panelBg, padding: 22, display: "grid", gap: 18 }}>
        <header style={{ display: "flex", justifyContent: "space-between", alignItems: "start" }}><h2 style={{ margin: 0, fontSize: "var(--fs-hero)" }}>{"name" in selected ? selected.name : selected.title}</h2>
          {props.canEdit ? <div style={{ display: "flex", gap: 8 }}><Button onClick={() => setEditor("edit")}>{translateUi("Edit")}</Button><Button variant="danger" onClick={async () => {
          ...
          }}>{translateUi("Delete")}</Button></div> : null}</header>
  ...
  return <div style={{ display: "grid", gap: 12 }}>
    <div style={{ display: "flex", gap: 8, justifyContent: "space-between", flexWrap: "wrap" }}>
      {props.type === "events" ? <div style={{ display: "flex", gap: 8, flex: 1, flexWrap: "wrap" }}>
        <Input value={timelineQuery} onChange={(event) => setTimelineQuery(event.target.value)} placeholder={translateUi("Filter timeline…")} style={{ maxWidth: 300 }} />
        <SearchableSelect value={timelineRecordId} onChange={setTimelineRecordId} options={records} placeholder={translateUi("All related records")} />
        <SearchableSelect value={timelineTag} onChange={setTimelineTag} options={timelineTags} placeholder={translateUi("All tags")} />
        <SearchableSelect value={timelineCampaignId} onChange={setTimelineCampaignId} options={props.campaigns} placeholder={translateUi("All Campaigns")} />
        <SearchableSelect value={timelinePlaceId} onChange={setTimelinePlaceId} options={places} placeholder={translateUi("All places")} />
      </div> : <span />}
      {props.canEdit ? <Button onClick={() => setEditor("new")}>{translateUi("+ New")} {singular}</Button> : null}
    </div>
    {props.type === "events" ? <div style={{ display: "grid", gap: 0, borderLeft: `2px solid ${withAlpha(props.accent, .45)}`, marginLeft: 72 }}>
      ...
      {!timelineEvents.length ? <div style={{ padding: 35, color: theme.colors.muted }}>{translateUi("No matching events.")}</div> : null}
      ...
      {!list.length ? <div style={{ padding: 40, textAlign: "center", color: theme.colors.muted }}>{translateUi("No")} {props.type} {translateUi("yet.")}</div> : null}
    </div>}
    {editor ? props.type === "items"
