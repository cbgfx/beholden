...
export function BinderMembersPanel({ binderId }: { binderId: string }) {
  ...
  return <section style={{ padding: 16, border: `1px solid ${theme.colors.panelBorder}`, borderRadius: theme.radius.panel, background: theme.colors.panelBg }}>
    <h2 style={{ margin: "0 0 12px" }}>{translateUi("Access")}</h2>
    <div style={{ display: "flex", gap: 8, marginBottom: 12 }}>
      <Input value={username} onChange={(event) => setUsername(event.target.value)} placeholder={translateUi("Username")} />
      <select value={role} onChange={(event) => setRole(event.target.value as typeof role)} style={{ padding: "0 10px", borderRadius: 8 }}>
        <option value="collaborator">{translateUi("Collaborator")}</option><option value="viewer">{translateUi("Viewer")}</option>
      </select>
      <Button onClick={async () => { try { setError(""); await saveBinderMember(binderId, username, role); setUsername(""); await reload(); } catch (e) { setError(e instanceof Error ? e.message : translateMessage("Could not update access")); } }} disabled={!username.trim()}>{translateUi("Add")}</Button>
    </div>
    ...
    <div style={{ display: "grid", gap: 6 }}>{members.map((member) => <div key={member.id} style={{ display: "flex", alignItems: "center", gap: 10 }}>
      ...
      {member.role !== "owner" ? <Button onClick={async () => { await removeBinderMember(binderId, member.id); await reload(); }}>{translateUi("Remove")}</Button> : null}
    </div>)}</div>
  </section>;
}
