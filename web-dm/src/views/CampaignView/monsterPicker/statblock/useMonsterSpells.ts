...
export function useMonsterSpells(monster: any | null): {
  ...
  const monsterRuleset: "5e" | "5.5e" | undefined = m?.ruleset === "5e" || m?.ruleset === "5.5e" ? m.ruleset : undefined;
  ...
  }, [m?.id, spellNames, monsterRuleset]);
  ...
  const openSpell = React.useCallback(async (ref: { id?: string | null; name: string }) => {
    ...
    try {
      ...
      if (directId) {
        const detail = await api<any>(`/api/spells/${directId}${rulesetParam}`);
      ...
      const detail = await api<any>(`/api/spells/${best.id}${rulesetParam}`);
      setSpellDetail(detail);
    } catch (e: any) {
