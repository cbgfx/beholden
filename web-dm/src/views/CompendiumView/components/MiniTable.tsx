...
export function MiniTable(props: { cols: string[]; rows: Array<Array<string | number>> }) {
  return (
    <div style={{ border: `1px solid ${theme.colors.panelBorder}`, borderRadius: 12, overflow: "hidden" }}>
      ...
      >
        {props.cols.map((c) => (
          <div key={c} style={{ padding: "8px 10px", fontWeight: 800, fontSize: "var(--fs-small)", color: theme.colors.muted }}>
            {c}
          </div>
        ))}
      </div>
      {props.rows.map((r, idx) => (
        ...
        >
          {r.map((cell, i) => (
            <div key={i} style={{ padding: "8px 10px", fontSize: "var(--fs-subtitle)", color: theme.colors.text }}>
              {cell}
            </div>
          ))}
        </div>
