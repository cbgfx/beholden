  ...
  const resolveMonsterId = React.useCallback(
    (c: Combatant | null): string | null => {
      ...
      const polymorphCond = c.conditions?.find((x) => x.key === "polymorphed") as
        ...
        | undefined;
      const polymorphMonsterId =
        typeof polymorphCond?.polymorphMonsterId === "string" && polymorphCond.polymorphMonsterId
          ? polymorphCond.polymorphMonsterId
          : null;
      // Polymorphed: use the form's monster if set, or null to hide original stats.
      if (polymorphCond) return polymorphMonsterId;
      if (c.baseType === "monster" && typeof c.baseId === "string") return c.baseId;
      if (c.baseType === "inpc") {
        const mid = inpcsById?.[String(c.baseId)]?.monsterId ?? null;
