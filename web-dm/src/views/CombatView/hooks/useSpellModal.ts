...
function monsterRulesetOf(monster: MonsterDetail | null): "5e" | "5.5e" | undefined {
  return monster?.ruleset === "5e" || monster?.ruleset === "5.5e" ? monster.ruleset : undefined;
}
...
export function useSpellModal(activeMonster: MonsterDetail | null, targetMonster: MonsterDetail | null) {
  ...
  // Load spell levels (light cache) so we can sort by spell level. Active and target monsters
  // can have different rulesets, so each side's names are resolved against its own monster's
  // ruleset -- a shared unscoped lookup could silently match the wrong ruleset's spell (and
  // therefore the wrong level) when a name exists in both.
  const activeRuleset = monsterRulesetOf(activeMonster);
  const targetRuleset = monsterRulesetOf(targetMonster);
  React.useEffect(() => {
    let alive = true;
    const resolveGroup = async (names: string[], ruleset: "5e" | "5.5e" | undefined) => {
      ...
      try {
        const payload = await api<{ rows: SpellLookupRow[] }>("/api/spells/lookup", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ names: unresolved, ruleset }),
        });
        if (!alive) return;
    ...
    void resolveGroup(activeSpellNames, activeRuleset);
    void resolveGroup(targetSpellNames, targetRuleset);
    return () => {
      alive = false;
    };
  }, [targetSpellNames, activeSpellNames, activeRuleset, targetRuleset]);
