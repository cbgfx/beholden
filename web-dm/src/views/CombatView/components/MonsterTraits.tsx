...
export function MonsterTraits(props: {
  ...
  const legendaryResistanceTrait = allTraits.find((t) =>
    /legendary resistance/i.test(String(t?.name ?? ""))
  );
  ...
  return (
    <div style={{ marginTop: 14 }}>
      {legendaryResistanceTrait && (
        <div style={{ marginBottom: otherTraits.length ? 14 : 0 }}>
          ...
          {resistanceText ? (
            <div style={{ color: theme.colors.text, opacity: 0.9, fontSize: "var(--fs-medium)", lineHeight: 1.3, background: theme.colors.panelBg, border: `1px solid ${theme.colors.panelBorder}`, borderRadius: 10, padding: 10 }}>
              {resistanceText}
            </div>
          ) : null}
        </div>
