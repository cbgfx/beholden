...
import { registerTreasureAwardRoute } from "./treasureAward.js";
...
export function hydrateTreasureEntry(db: Database.Database, entry: ReturnType<typeof rowToTreasure>) {
...
export function registerTreasureRoutes(app: Express, ctx: ServerContext) {
  ...
  app.get("/api/campaigns/:campaignId/treasure", memberOrAdmin(db), (req, res) => {
    ...
    const treasure = rows.map(rowToTreasure).map((entry) => hydrateTreasureEntry(db, entry));
  ...
  app.patch("/api/treasure/:treasureId/qty", dmOrAdmin(db), (req, res) => {
    ...
    const treasure = hydrateTreasureEntry(db, rowToTreasure(row));
    const t = now();
    db.prepare(
    ...
    const dto = toTreasureDto(hydrateTreasureEntry(db, rowToTreasure(updatedRow)));
    emitTreasureChange({
  ...
  app.post("/api/campaigns/:campaignId/treasure", dmOrAdmin(db), (req, res) => {
    ...
    const dto = toTreasureDto(hydrateTreasureEntry(db, out.entry));
    emitTreasureChange({
      campaignId,
      adventureId: null,
      encounterId: null,
