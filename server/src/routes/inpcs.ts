...
export function registerInpcRoutes(app: Express, ctx: ServerContext) {
  ...
  app.post("/api/campaigns/:campaignId/inpcs", (req, res) => {
    ...
    const defaultAcDetail =
      (m?.ac?.note ?? m?.ac?.type ?? (defaultAcRaw != null ? String(defaultAcRaw).replace(/^(-?\d+)\s*/, "").trim() : null)) ||
      null;
    const defaultHpDetail =
      (m?.hp?.formula ?? m?.hp?.roll ?? (defaultHpRaw != null ? String(defaultHpRaw).replace(/^(-?\d+)\s*/, "").trim() : null)) ||
      null;
    ...
    for (let i = 0; i < qty; i++) {
      ...
      const hpMax =
        b.hpMax != null && Number.isFinite(Number(b.hpMax)) ? Number(b.hpMax) : defaultHp != null ? defaultHp : 1;
