...
-- many same-named spells (e.g. Fireball). id alone is NOT unique across rulesets, same as
-- classes/species/backgrounds/feats below -- the primary key is composite so both can coexist.
...
-- Like spells, maneuvers/metamagic/invocations/etc. genuinely differ in text and mechanics
-- between 2014 and 2024 for many same-id talents (e.g. ct_maneuver_ambush). id alone is NOT
-- unique across rulesets -- the primary key is composite so both can coexist.
