...
const legacySpellsPath = path.join(compendiumDir, "legacy_spells.json");
...
const legacySpellsDoc = fs.existsSync(legacySpellsPath) ? JSON.parse(fs.readFileSync(legacySpellsPath, "utf8")) : null;
...
if (legacySpellsDoc) importNativeCompendiumDocument(db, legacySpellsDoc);
