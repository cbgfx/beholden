  ...
  for (const batch of batches) for (const entry of batch.entries) {
    ...
    if (batch.category === "classes") {
      ...
      if (existing?.data_json) {
        ...
        if (existingLevels === 20 && incomingLevels < 20) {
          const incomingLevelNumbers = new Set(
            (entry.levels as JsonRecord[] | undefined ?? [])
          ...
          const missingLevels = Array.from(
            { length: 20 },
            (_, index) => index + 1,
          ).filter((level) => !incomingLevelNumbers.has(level));
          const missingLabel = missingLevels.length > 0
            ? `; missing level${missingLevels.length === 1 ? "" : "s"} ${missingLevels.join(", ")}`
            : "";
          issues.push(`${owner}.levels is a partial replacement (${incomingLevels}/20 levels${missingLabel})`);
        }
      }
    }
